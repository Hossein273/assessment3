...
const { requireGroups, getGroups } = require("./permissions");
const {
  getVideoById,
  deleteVideoRecord,
  getUserVideos,
...
app.post("/upload/trigger/:id", authenticateToken, async (req, res) => {
  ...
  try {
    ...
    res.json({ message: "Transcoding queued", videoId: id });
  } catch (err) {
    console.error("Trigger error:", err);
    res.status(500).json({ error: "Failed to queue transcoding" });
  }
});
...
app.get("/videos/:id/status", authenticateToken, async (req, res) => {
  try {
    ...
    const status = (video.status || "UNKNOWN").toUpperCase();
    res.json({ status });
  } catch (err) {
