...
async function updateVideoStatus(username, videoId, status) {
  console.log(
    `[DEBUG] Updating DynamoDB → ${TABLE_NAME} for ${username}/${videoId}`
  );
  ...
  try {
    ...
    console.log(
      `[DEBUG] DynamoDB updated item:`,
      JSON.stringify(result.Attributes, null, 2)
    );
  } catch (err) {
    console.error(
      `[ERROR] DynamoDB update failed for ${username}/${videoId}:`,
      err
    );
  }
}
...
async function processMessage(msg) {
  ...
  console.log("DEBUG job message:", body);

  const { username, videoId } = body;
  const s3Key = body.key || body.rawKey || body.s3Key; // ✅ handle all cases
  ...
  try {
    ...
    const outputKey = s3Key.replace("raw/", "processed/"); // ✅ correct path
    ...
    await updateVideoStatus(username, videoId, "failed"); // ✅ fix args
    throw err;
  }
}
// ---------------------------------------------
